package com.shan.sample.paintjob;
...
public class PaintJob {
	...
	public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
		...
		double totalPaintBucket = calculateTotalBucket(width, height, areaPerBucket, extraBuckets);
		
		return (int) totalPaintBucket - extraBuckets;
	}
	...
	public static int getBucketCount(double width, double height, double areaPerBucket) {
		...
		double totalPaintBucket = calculateTotalBucket(width, height, areaPerBucket, 0);
		
		return (int) totalPaintBucket;
	}
	
	public static int getBucketCount(double areaOfTheWall, double areaPerBucket) {
		//If one of the parameters width, height or areaPerBucket is less or equal to 0 
		// ( or ) 
		//if extraBuckets is less than 0, the method needs to return -1 to indicate an invalid value.
		if((areaOfTheWall <= 0) || (areaPerBucket <= 0))
			return -1;
		
		double totalPaintBucket = Math.ceil(areaOfTheWall / areaPerBucket);
		
		return (int) totalPaintBucket;
	}
	
	public static double calculateTotalBucket(double width, double height, double areaPerBucket, int extraBuckets) {
		double areaOfTheWall = 0d;
		areaOfTheWall = width * height;
		return Math.ceil(areaOfTheWall / areaPerBucket);
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(getBucketCount(7.25, 4.3, 2.35));    // → should return 14 since the wall area is 31.175, and a single bucket can cover an area of 2.35.  // 13.26595
